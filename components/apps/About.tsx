            ...
            <div className="flex-1 overflow-auto p-6 md:p-10 custom-scrollbar">
                <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-10 md:gap-16">
                    ...
                    <div className="space-y-6">
                        <div className={`${bgTertiary} p-6 rounded-[32px] border ${borderStyle} shadow-sm`}>
                            <h3 className={`text-[10px] font-black ${textMuted} uppercase tracking-[0.25em] mb-6`}>{lang === 'tr' ? 'İLETİŞİM' : 'CONTACT'}</h3>
                            <div className="space-y-4">
                                <a href={`mailto:${d.email}`} className={`flex items-center gap-3 p-3 rounded-2xl ${isDarkMode ? 'bg-white/5 hover:bg-white/10' : 'bg-white hover:bg-black/5'} border ${borderStyle} transition-all group`}>
                                    <div className={`w-10 h-10 rounded-xl ${isDarkMode ? 'bg-blue-500/20' : 'bg-blue-50'} flex items-center justify-center shrink-0 group-hover:bg-blue-500 transition-colors`}>
                                        <Mail size={18} className={`${isDarkMode ? 'text-blue-400' : 'text-blue-600'} group-hover:text-white transition-colors`} />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className={`text-[10px] font-black ${textMuted} uppercase tracking-wider mb-0.5`}>{lang === 'tr' ? 'E-posta' : 'Email'}</p>
                                        <p className={`text-[11px] font-bold ${textColor}/80 overflow-hidden text-ellipsis whitespace-nowrap`}>{d.email}</p>
                                    </div>
                                </a>
                            </div>
                        </div>
                        ...
                        <div className={`${bgTertiary} p-6 rounded-[32px] border ${borderStyle} shadow-sm`}>
                            <h3 className={`text-[10px] font-black ${textMuted} uppercase tracking-[0.25em] mb-6`}>{lang === 'tr' ? 'BİLGİLER' : 'INFORMATION'}</h3>
                            <div className="space-y-4">
                                <div className={`flex items-start gap-3 p-3 rounded-2xl ${isDarkMode ? 'bg-white/5' : 'bg-white'} border ${borderStyle}`}>
                                    <div className={`w-10 h-10 rounded-xl ${isDarkMode ? 'bg-green-500/20' : 'bg-green-50'} flex items-center justify-center shrink-0`}>
                                        <MapPin size={18} className={`${isDarkMode ? 'text-green-400' : 'text-green-600'}`} />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className={`text-[10px] font-black ${textMuted} uppercase tracking-wider mb-0.5`}>{lang === 'tr' ? 'Konum' : 'Location'}</p>
                                        <p className={`text-[13px] font-bold ${textColor}/80`}>{d.location}</p>
                                    </div>
                                </div>

                                <div className={`flex items-start gap-3 p-3 rounded-2xl ${isDarkMode ? 'bg-white/5' : 'bg-white'} border ${borderStyle}`}>
                                    <div className={`w-10 h-10 rounded-xl ${isDarkMode ? 'bg-purple-500/20' : 'bg-purple-50'} flex items-center justify-center shrink-0`}>
                                        <GraduationCap size={18} className={`${isDarkMode ? 'text-purple-400' : 'text-purple-600'}`} />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className={`text-[10px] font-black ${textMuted} uppercase tracking-wider mb-0.5`}>{lang === 'tr' ? 'Eğitim' : 'Education'}</p>
                                        <p className={`text-[13px] font-bold ${textColor}/80 leading-tight`}>{d.education}</p>
                                    </div>
                                </div>
                            </div>
                        </div>

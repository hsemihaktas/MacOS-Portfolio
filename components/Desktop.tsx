...
const Desktop: React.FC<DesktopProps> = ({ onIconClick, isMobile, isTablet, lang, getAppTitle }) => {
    const containerClass = isMobile
        ...
        : isTablet
            ...
            : "absolute inset-0 pt-12 pb-24 px-8 flex flex-col flex-wrap content-end gap-x-12 gap-y-10";

    const iconSize = isMobile ? "w-[62px] h-[62px]" : isTablet ? "w-[72px] h-[72px]" : "w-[54px] h-[54px]";
    const iconRadius = isMobile || isTablet ? "rounded-[14px]" : "rounded-none";

    return (
        <div className={`${containerClass} select-none h-full w-full pointer-events-none`}>
            {APPS.map((app) => (
                <div
                    key={app.id}
                    className="group flex flex-col items-center gap-1.5 cursor-default transition-all duration-300 pointer-events-auto"
                ...
                >
                    ...
                    <span className={`text-white text-center leading-tight truncate px-1.5 py-0.5 rounded-[5px] transition-all
            ${isMobile || isTablet

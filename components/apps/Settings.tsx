    ...
    switch (activeSection) {
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-right-2 duration-300">
            <div>
              ...
              <div className="grid grid-cols-2 gap-8">
                ...
                >
                  <div className={`w-full aspect-[4/3] rounded-xl border-4 transition-all overflow-hidden ${!isDarkMode ? 'border-blue-500 shadow-lg' : 'border-transparent opacity-60 hover:opacity-100 bg-white shadow-sm'}`}>
                ...
                <div 
                  ...
                  className="flex flex-col items-center gap-3 cursor-default group"
                >
                  <div className={`w-full aspect-[4/3] rounded-xl border-4 transition-all overflow-hidden ${isDarkMode ? 'border-blue-500 shadow-lg' : 'border-transparent opacity-60 hover:opacity-100 bg-[#1e1e1e] shadow-sm'}`}>
                  ...
                  <span className={`text-[13px] font-bold ${textColor}`}>{lang === 'tr' ? 'Koyu' : 'Dark'}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      case 'display':
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-2 duration-300">
            <div>
              <h2 className={`text-[20px] font-black mb-6 ${textColor}`}>{lang === 'tr' ? 'Ekran' : 'Display'}</h2>
              <div className={`p-6 rounded-2xl ${isDarkMode ? 'bg-white/5' : 'bg-[#fcfcfc]'} border ${borderStyle} shadow-sm`}>
                <div className="flex justify-between items-center mb-4">
                  <span className={`text-[14px] font-bold ${textColor} opacity-60`}>{lang === 'tr' ? 'Parlaklık' : 'Brightness'}</span>
                  <span className={`text-[14px] font-black ${textColor}`}>{brightness}%</span>
                </div>
      ...
      case 'wifi':
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-2 duration-300">
            <div>
              <h2 className={`text-[20px] font-black mb-6 ${textColor}`}>Wi-Fi</h2>
              <div className={`p-4 rounded-2xl ${isDarkMode ? 'bg-white/5' : 'bg-[#fcfcfc]'} border ${borderStyle} shadow-sm flex items-center justify-between`}>
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isWifiEnabled ? 'bg-blue-500 text-white' : 'bg-black/10 text-black/40'}`}>
                    <Wifi size={20} />
                  </div>
                  <div>
                    <div className={`text-[14px] font-bold ${textColor}`}>Wi-Fi</div>
                ...
                >
                  <div className={`w-4 h-4 bg-white rounded-full transition-all ${isWifiEnabled ? 'ml-6' : 'ml-0'} shadow-md`}></div>
                </button>
              </div>
            </div>
          </div>
                ...
                {['tr', 'en'].map((l, idx) => (
                  ...
                  >
                    <div className="flex flex-col items-start">
                      <span className={`text-[14px] font-bold ${textColor}`}>{l === 'tr' ? 'Türkçe' : 'English'}</span>
                      <span className={`text-[11px] font-medium ${textColor} opacity-40 uppercase tracking-widest`}>{l}</span>
                    </div>
                    {lang === l && <Check size={18} className="text-blue-500" />}
                  </button>
                ))}
              </div>
            </div>
      ...
      <div className={`w-[240px] ${bgSidebar} border-r ${borderStyle} flex flex-col pt-6 shrink-0`}>
        ...
        <div className="flex-1 overflow-auto px-3 space-y-1">
          {sidebarItems.map((item) => (
            ...
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-white shadow-sm ${activeSection === item.id && !isDarkMode ? item.color : (activeSection === item.id && isDarkMode ? 'bg-white/20' : item.color)}`}>
                {item.icon}
              </div>
              <span className="text-[13px] font-bold">{item.label}</span>
              {activeSection !== item.id && <ChevronRight size={14} className="ml-auto opacity-20" />}
            </button>
          ))}
        </div>
      </div>

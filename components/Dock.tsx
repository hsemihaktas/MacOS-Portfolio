'use client';

import React from 'react';
import { APPS } from '@/lib/constants';
import { AppID, WindowState, Language } from '@/lib/types';
import { Github, Linkedin } from 'lucide-react';

interface DockProps {
    onAppClick: (id: AppID) => void;
    activeApp: AppID | null;
    windows: WindowState[];
    onLaunchpadClick: () => void;
    isMobile?: boolean;
    isTablet?: boolean;
    lang: Language;
    getAppTitle: (id: AppID) => string;
}

const Dock: React.FC<DockProps> = ({ onAppClick, activeApp, windows, onLaunchpadClick, isMobile, isTablet, lang, getAppTitle }) => {
    if (isMobile) {
        return (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-[90%] z-[9998]">
                <div className="bg-white/20 backdrop-blur-3xl rounded-[32px] p-4 flex justify-between items-center shadow-2xl border border-white/10 ring-1 ring-white/5">
                    {APPS.slice(0, 4).map((app) => (
                        <div
                            key={app.id}
                            className="w-[62px] h-[62px] active:scale-90 transition-transform"
                            onClick={() => onAppClick(app.id)}
                        >
                            <img src={app.iconUrl} alt={getAppTitle(app.id)} className="w-full h-full rounded-[15px] shadow-lg" />
                        </div>
                    ))}
                </div>
            </div>
        );
    }

    if (isTablet) {
        return (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[9998]">
                <div className="bg-white/15 backdrop-blur-3xl rounded-[28px] px-5 py-3 flex gap-4 items-center shadow-2xl border border-white/10 ring-1 ring-white/5">
                    {APPS.map((app) => (
                        <div
                            key={app.id}
                            className="w-[64px] h-[64px] active:scale-90 transition-transform"
                            onClick={() => onAppClick(app.id)}
                        >
                            <img src={app.iconUrl} alt={getAppTitle(app.id)} className="w-full h-full rounded-[15px] shadow-lg" />
                        </div>
                    ))}
                </div>
            </div>
        );
    }

    return (
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 p-2 z-[9999]">
            <div className="flex items-end gap-3 bg-white/10 backdrop-blur-3xl border border-white/20 rounded-[22px] px-3 py-2 shadow-[0_30px_60px_rgba(0,0,0,0.4)] ring-1 ring-black/5">

                <div className="relative group cursor-default flex flex-col items-center" onClick={onLaunchpadClick}>
                    <div className="absolute -top-[55px] left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur text-[#1d1d1f] text-[12px] font-bold px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 whitespace-nowrap shadow-xl pointer-events-none transform translate-y-2 group-hover:translate-y-0 border border-black/5">
                        Launchpad
                    </div>
                    <div className="w-[50px] h-[50px] transition-all duration-300 transform group-hover:scale-[1.3] group-hover:-translate-y-4 group-active:scale-95 group-active:translate-y-0 origin-bottom ease-[cubic-bezier(0.25,1,0.5,1)]">
                        <img
                            src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c9/Launchpad_macOS_Big_Sur.png/512px-Launchpad_macOS_Big_Sur.png"
                            alt="Launchpad"
                            className="w-full h-full object-contain"
                        />
                    </div>
                </div>

                {APPS.map((app) => {
                    const isOpen = windows.find(w => w.id === app.id)?.isOpen;
                    return (
                        <div key={app.id} className="relative group cursor-default flex flex-col items-center" onClick={() => onAppClick(app.id)}>
                            <div className="absolute -top-[55px] left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur text-[#1d1d1f] text-[12px] font-bold px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 whitespace-nowrap shadow-xl pointer-events-none transform translate-y-2 group-hover:translate-y-0 border border-black/5">
                                {getAppTitle(app.id)}
                            </div>
                            <div className="w-[50px] h-[50px] transition-all duration-300 transform group-hover:scale-[1.3] group-hover:-translate-y-4 group-active:scale-95 group-active:translate-y-0 origin-bottom ease-[cubic-bezier(0.25,1,0.5,1)]">
                                <img src={app.iconUrl} alt={getAppTitle(app.id)} className="w-full h-full object-contain drop-shadow-md" draggable={false} />
                            </div>
                            {isOpen && <div className="absolute -bottom-1 w-[4px] h-[4px] bg-white rounded-full shadow-sm"></div>}
                        </div>
                    );
                })}

                <div className="w-[1px] h-10 bg-white/20 mx-1.5 self-center"></div>

                <div className="flex items-center gap-2">
                    <a href="https://github.com" target="_blank" className="group relative w-[48px] h-[48px] bg-black/10 hover:bg-black/20 backdrop-blur-xl rounded-xl flex items-center justify-center transition-all duration-300 hover:scale-[1.2] hover:-translate-y-4 origin-bottom">
                        <Github size={24} className="text-white/80" />
                    </a>
                    <a href="https://linkedin.com" target="_blank" className="group relative w-[48px] h-[48px] bg-[#0077b5]/20 hover:bg-[#0077b5]/30 backdrop-blur-xl rounded-xl flex items-center justify-center transition-all duration-300 hover:scale-[1.2] hover:-translate-y-4 origin-bottom border border-[#0077b5]/10">
                        <Linkedin size={24} className="text-white/80 fill-white/20" />
                    </a>
                </div>
            </div>
        </div>
    );
};

export default Dock;
